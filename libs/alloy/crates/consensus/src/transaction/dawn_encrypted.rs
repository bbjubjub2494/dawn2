...
use dawn_crypto::{Ciphertext, EphemeralPublicKey};
...
pub struct TxDawnEncrypted {
    ...
    /// contains recipient address and input data.
...
impl TxDawnEncrypted {
    ...
    pub fn fields_len(&self) -> usize {
        ...
        len += self.ciphertext.u.0.length();
        len += self.ciphertext.payload.length();
        len += self.ciphertext.tag.length();
        len
    }
...
impl Transaction for TxDawnEncrypted {
...
impl SignableTransaction<Signature> for TxDawnEncrypted {
...
impl Encodable for TxDawnEncrypted {
...
impl Decodable for TxDawnEncrypted {
