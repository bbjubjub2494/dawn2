...
use dawn_crypto::{DecryptionKey, EphemeralPublicKey};

#[derive(Clone, Debug, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(feature = "serde", serde(rename_all = "camelCase"))]
#[doc(
    alias = "DawnDecryptedTransaction",
    alias = "TransactionDawnDecrypted",
    alias = "DawnDecryptedTx"
)]
pub struct TxDawnDecrypted {
...
impl TxDawnDecrypted {
    ...
    /// Decodes the inner [TxDawnDecrypted] fields from RLP bytes.
    ///
    ...
    pub fn decode_fields(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        Ok(Self {
            ...
            ephemeral_public_key: EphemeralPublicKey(Decodable::decode(buf)?),
            decryption_key: DecryptionKey(Decodable::decode(buf)?),
    ...
    pub fn fields_len(&self) -> usize {
        ...
        len += self.ephemeral_public_key.0.length();
        len += self.decryption_key.0.length();
        len += self.sender.length();
        len
    }
    ...
    pub(crate) fn encode_fields(&self, out: &mut dyn alloy_rlp::BufMut) {
        ...
        out.put_slice(&self.ephemeral_public_key.0);
        out.put_slice(&self.decryption_key.0);
    ...
    /// Calculates a heuristic for the in-memory size of the [TxDawnDecrypted] transaction.
    #[inline]
    pub fn size(&self) -> usize {
        ...
        mem::size_of::<EphemeralPublicKey>() + // value
        mem::size_of::<DecryptionKey>() // value
    }
}

impl Transaction for TxDawnDecrypted {
...
impl SignableTransaction<Signature> for TxDawnDecrypted {
...
impl Encodable for TxDawnDecrypted {
...
impl Decodable for TxDawnDecrypted {
